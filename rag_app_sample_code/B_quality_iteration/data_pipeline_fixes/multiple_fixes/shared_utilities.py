...
def get_strategy_packed_json_string(baseline_strategy, strategy_to_try):
    ...
    merged_strategy["destination_tables_config"] = {
        ...
        "raw_files_table_name": f"`{UC_CATALOG}`.`{UC_SCHEMA}`.`{strategy_name}_raw_files_bronze`".replace("`", ""),
        # Parsed documents
        "parsed_docs_table_name": f"`{UC_CATALOG}`.`{UC_SCHEMA}`.`{strategy_name}_parsed_docs_silver`".replace("`", ""),
        # Chunked documents that are loaded into the Vector Index
        "chunked_docs_table_name": f"`{UC_CATALOG}`.`{UC_SCHEMA}`.`{strategy_name}_chunked_docs_gold`".replace("`", ""),
        # Destination Vector Index
        "vectorsearch_index_name": f"{UC_CATALOG}.{UC_SCHEMA}.{strategy_name}_chunked_docs_gold_index".replace("`", ""),
